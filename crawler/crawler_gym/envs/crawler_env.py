...
class CrawlerEnv(gym.Env):
    ...
    def __init__(self, headless=False, push_robot=False, add_noise=False, add_bias=False, resample_cmd=True, random_orient=True):
        self.step_counter = 0
    ...
    def _reward_exp(self):
        r_v = 0.9 * np.exp(-(self.commands[0] - self.crawler.get_state()[7])**2/(0.001)) + 0.1 * np.exp(-(self.commands[0] - self.crawler.get_state()[7])**2/(100*0.001))
        r_w = 0.9 * np.exp(-(self.commands[1] - self.crawler.get_state()[12])**2/(0.01)) + 0.1 * np.exp(-(self.commands[1] - self.crawler.get_state()[12])**2/(100*0.01))
        return r_v * r_w
